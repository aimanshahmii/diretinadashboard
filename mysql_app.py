...
from utils.mysql_database import (
    add_mysql_prediction as add_prediction,
    check_mysql_db_connection as check_db_connection
)

# Import the MySQL database functions
from utils.mysql_database import (
    init_mysql_db,
    get_all_mysql_predictions, 
    get_all_mysql_patients,
    add_mysql_patient
)

# Override the database functions in the database.py
import utils.database
utils.database.init_db = init_mysql_db
utils.database.get_all_predictions = get_all_mysql_predictions
utils.database.get_all_patients = get_all_mysql_patients
utils.database.add_patient = add_mysql_patient

# Page configuration
st.set_page_config(
    page_title="DiRetina Dashboard (MySQL)",
    page_icon="👁️",
    layout="wide",
...
if "app_version" not in st.session_state:
    ...
    st.session_state.mysql_url = None
    ...
    if "show_db_config" not in st.session_state:
        st.session_state.show_db_config = False
    if "mysql_url" not in st.session_state:
        st.session_state.mysql_url = None
...
def main():
    ...
    with st.sidebar:
        ...
        # Database URL input
        if st.button("Configure MySQL Connection"):
            st.session_state.show_db_config = not st.session_state.show_db_config
            
        if st.session_state.show_db_config:
            st.markdown("#### MySQL Connection")
            mysql_url = st.text_input(
                "MySQL Connection URL", 
                value=st.session_state.mysql_url if st.session_state.mysql_url else "",
                placeholder="mysql://username:password@host:port/database",
                type="password"
            )
            
            if st.button("Save MySQL Configuration"):
                st.session_state.mysql_url = mysql_url
                # Test connection
                if check_db_connection(_show_messages=True):
                    st.session_state.show_db_config = False
                else:
                    st.error("Could not connect to MySQL with the provided URL")
